...
**Draft Step:** The GPU acts as the primary computing device. Inference is accelerated by maximizing CPU participation and minimizing parameter transfer. Specifically, for experts activated on the CPU, use a **Draft Scheduler** to select the top-*c* experts (initially setting *c=2*) to run locally on the CPU. For the remaining experts, select substitute experts present in GPU memory to run on the GPU.
