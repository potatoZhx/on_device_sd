...
def main():
    ...
    draft_tokens = torch.randint(0, 100, (args.draft_tokens,), device="cuda").tolist()
    verify_input = torch.tensor([1] + draft_tokens, device="cuda", dtype=torch.long)
    positions = torch.arange(0, 1 + args.draft_tokens, device="cuda", dtype=torch.long)

    torch.cuda.synchronize()
    start = time.perf_counter()
    for _ in range(args.iters):
        executor._forward_verify(verify_input, positions, [seq_id])
    torch.cuda.synchronize()
    elapsed = time.perf_counter() - start
    total_tokens = args.iters * args.draft_tokens
    throughput = total_tokens / elapsed if elapsed > 0 else 0.0
    print(f"verify_tokens={total_tokens} time={elapsed:.4f}s throughput={throughput:.2f} tok/s")
